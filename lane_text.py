...
#输入原图宽度、高度、4个梯形点
# 保存原始梯形点
# 把4个点转成 OpenCV 需要的 float32 格式
# 根据 pt1、pt2、pt4 计算目标矩形的宽度和高度范围
# 构造目标矩形 dst
# 计算原图 → 鸟瞰图的透视变换矩阵
# 计算鸟瞰图 → 原图的反向透视变换矩阵
# 保存变换后图像尺寸和原图尺寸
...
# 定义函数 warp，输入一张图像和文件名：
#     复制原图，得到一张用于调试显示的图像
#     在调试图像上画出目标矩形区域，颜色为白色
#     在调试图像上画出原始梯形区域，颜色为绿色
#     保存这张调试图像，文件名前缀为 persp_
#     对原始图像做透视变换，得到鸟瞰图
#     保存鸟瞰图，文件名前缀为 warp_
#     返回鸟瞰图
...
#对多个历史检测到的车道线 x 坐标求平均，降低单帧误差，让车道线检测结果更加稳定
...
def slide_window(img, binary_warped, hist, num_windows, filename=None, prefix=None):
    ...
    for idx_window in range(num_windows):
        ...
        non_zero_left = ((non_zero_y >= win_y_bottom) & (non_zero_y < win_y_top) &
                         (non_zero_x >= win_x_left_min) & (non_zero_x < win_x_left_max)).nonzero()[0]

        non_zero_right = ((non_zero_y >= win_y_bottom) & (non_zero_y < win_y_top) &
                          (non_zero_x >= win_x_right_min) & (non_zero_x < win_x_right_max)).nonzero()[0]
